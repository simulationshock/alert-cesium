...
export async function resolveOrRegisterUser(store, profile, requestId) {
    if (!profile.subject)
        throw new AuthUserError('missing_provider_subject');
    ...
    if (verifiedEmail) {
        ...
        if (matches.length !== activeMatches.length)
            throw new AuthUserError('disabled_user');
        if (activeMatches.length > 1)
            throw new AuthUserError('duplicate_identity_conflict');
        user = activeMatches[0];
    }
    if (!user) {
        if (!verifiedEmail)
            throw new AuthUserError('missing_verified_identity');
        user = {
            id: createId('usr'),
            status: 'active',
...
async function requireActiveUser(store, userId) {
    const user = await store.findUserById(userId);
    if (!user)
        throw new AuthUserError('persistence_failure');
    if (user.status === 'disabled')
        throw new AuthUserError('disabled_user');
    if (user.status === 'blocked')
        throw new AuthUserError('blocked_user');
    return user;
}
